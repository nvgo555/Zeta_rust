...
$$Q_c = \text{ring\textunderscore lin}(X_c^{(k)}, W_c^Q), \quad K_c = \text{ring\textunderscore lin}(X_c^{(k)}, W_c^K), \quad V_c = \text{ring\textunderscore lin}(X_c^{(k)}, W_c^V)$$
...
$$\mathrm{Out} = \text{ring\textunderscore lin}([O{\mathrm{dom}}; O{\mathrm{sub}}], W_O)$$
...
$$\mathrm{head} = \sum{k=0}^{\mathrm{prec}-1} (k+1) \cdot \text{to\textunderscore ring}\big(\text{witt\textunderscore head}[\dots, k, :]\big) \pmod{13}$$
...
- **$S_3$ parallel orbit:** Cycled when error rate exceeds $50\%$ ($\text{avg\textunderscore err} > 12$ out of $24$ steps).
...
| Witt | A026–A027 | Witt addition and multiplication are consistent with ring arithmetic: $\text{to\textunderscore ring}(\mathrm{wadd}(w, v)) = a + b$, $\text{to\textunderscore ring}(\mathrm{wmul}(w, v)) = a \cdot b$ |
