...
$$Q_c = \text{ring\_lin}(X_c^{(k)}, W_c^Q), \quad K_c = \text{ring\_lin}(X_c^{(k)}, W_c^K), \quad V_c = \text{ring\_lin}(X_c^{(k)}, W_c^V)$$
...
$$\mathrm{Out} = \text{ring\_lin}([O_{\mathrm{dom}}; O_{\mathrm{sub}}], W_O)$$
...
$$\mathrm{head} = \sum_{k=0}^{\mathrm{prec}-1} (k+1) \cdot \text{to\_ring}\big(\text{witt\_head}[\dots, k, :]\big) \pmod{13}$$
